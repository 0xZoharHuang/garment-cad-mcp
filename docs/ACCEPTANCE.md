...
| 4. Tape change triggers parametric recalculation | Native `.vit`/`.vst` lifecycle, personal/file metadata, dimension labels and restrictions, corrections/value aliases, plus a 100 mm to 200 mm measurement-driven piece snapshot test | Passing |
| 5. Puzzle nesting and all implemented exports | Official MaleShirt layout, reopen, and runtime validation of all 24 implemented `LayoutExportFormats` values; upstream `NC` remains explicitly reserved | Passing |
